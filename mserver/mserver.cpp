...
void Mserver::bindingProcess(int* sockfd, int* portno, struct sockaddr_in* serv_addr){
...
string Mserver::getVariableValue(string variableName){
...
bool Mserver::checkBooleanValueOfExpression(string stringExpression){
...
double Mserver::checkNumericalValueOfExpression(string stringExpression){
    QString qExpression = QString::fromStdString(stringExpression);
       qExpression.remove(" ");
       long double ld;
       if(qExpression.contains("+")){
...
vector<string> Mserver::removeScopeMemory(){
                        ...
                        if(nameToTypeMap.count(jsonBuffer["value"]) > 0){
                            if (refToVariableType[jsonBuffer["name"]] == nameToTypeMap[jsonBuffer["value"]]){
                                ...
                                if(jsonBuffer["getAdressFlag"] == "true"){
                                    ...
                                    resetMemory();
                                    return;
                                } else if (refToVariableType[jsonBuffer["name"]] == refToVariableType[jsonBuffer["value"]]){
                                    string refName = jsonBuffer["name"];
